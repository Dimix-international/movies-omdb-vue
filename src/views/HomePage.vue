...
export default {
  name: "HomePage",
  setup() {
    const search = ref(localStorage.getItem('search_movies') || '');
    const movies = ref([]);

    onMounted(async () => {
      const saveMovie = localStorage.getItem('search_movies');

      if(saveMovie) {
        try {
          const {data} =  await axios.get(`http://www.omdbapi.com/?apikey=b4ae785f&s=${saveMovie}`);
          const {Response} = data;
    ...
    const searchMovies = async() => {
      ...
      try {
        const {data} =  await axios.get(`http://www.omdbapi.com/?apikey=b4ae785f&s=${search.value}`);
        const {Response} = data;

        if(Response === 'True') {
          const {Search} = data;
          localStorage.setItem('search_movies', search.value);
          movies.value = Search;
        } else {
          const {Error} = data;
          alert(Error);
